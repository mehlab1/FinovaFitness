-- Online Store System Schema for Finova Fitness
...
CREATE TABLE IF NOT EXISTS store_carts (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, -- NULL for guest users
    guest_email VARCHAR(255), -- For guest users
    guest_name VARCHAR(255), -- For guest users
    guest_phone VARCHAR(20), -- For guest users
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS store_order_status_history (
    ...
    notes TEXT,
    changed_by INTEGER REFERENCES users(id) ON DELETE SET NULL, -- Admin who changed status
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS store_inventory_transactions (
    ...
    reference_type VARCHAR(50), -- 'order', 'admin_adjustment', 'return'
