...
export const TrainersTab = ({ showToast, onNavigateToReviews }: { showToast: (message: string, type?: 'success' | 'error' | 'info') => void; onNavigateToReviews?: (session: any) => void }) => {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Session request sent successfully! Waiting for trainer confirmation.', 'success');
      setShowBookingForm(false);
      setBookingForm({
        trainerId: 0,
        sessionDate: '',
      ...
      // Refresh sessions and available slots
      fetchSessions();
      fetchAvailableSlots();
      
    } catch (error) {
      console.error('Failed to book session:', error);
      showToast('Failed to send session request', 'error');
    } finally {
      ...
      <div className="flex justify-center">
        <div className="bg-gray-800 rounded-xl p-1 flex space-x-1">
          ...
          >
            ...
            <span className="font-medium text-sm">Trainers</span>
          </Button>
          <Button
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            ...
                            {upcomingSessions.filter(s => s.trainer_id === trainer.id).length > 0 && (
                              <>
                                <span className="text-xs text-gray-500">•</span>
                                <span className="text-xs text-blue-400">
                                  {upcomingSessions.filter(s => s.trainer_id === trainer.id).length} upcoming
                                </span>
                              </>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-4 text-sm">
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-green-400 font-bold">
                                {upcomingSessions.filter(s => s.trainer_id === selectedTrainer.id).length}
                              </p>
                      ...
                      <CardContent>
                        {availableSlots.length > 0 ? (
                          <>
                            <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                              <p className="text-sm text-blue-300 text-center">
                                💡 <strong>Available Session Times:</strong> These are the actual available training session slots. 
                                Break times between sessions are not shown as they are not bookable.
                              </p>
                            </div>
                          ...
                          <div className="text-center py-8 text-gray-400">
                            ...
                            <p className="text-sm">
                              {selectedDate ? 
                                'This trainer may not be working on this day, or all slots are already booked.' :
        ...
        <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700 shadow-xl">
          ...
          <CardContent>
            {upcomingSessions.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-400">
                <Calendar className="w-12 h-12 mx-auto mb-3 text-gray-500" />
                <p className="text-base">No upcoming sessions found.</p>
                <p className="text-sm">Book a session with a trainer to see it here.</p>
              </div>
            )}
          </CardContent>
        </Card>
